...
if (isset($_GET['api']) && $_GET['api'] == 'true') {
    ...
    try {
        if ($action == 'fetch_all') {
            $result = $conn->query("SELECT id, username, role, status FROM user WHERE role != 'superadmin' ORDER BY id DESC");
        ...
        if ($action == 'fetch_single') {
            $id = intval($_GET['id']);
            $stmt = $conn->prepare("SELECT id, username, role, status FROM user WHERE id=? AND role != 'superadmin'");
        ...
        if ($action == 'save') {
            ...
            if ($role == 'superadmin') throw new Exception('ไม่สามารถกำหนดสิทธิ์ Super Admin ได้');
        ...
        if ($action == 'delete') {
            $id = intval($_POST['id']);
            $stmt = $conn->prepare("DELETE FROM user WHERE id=? AND role != 'superadmin'");
            $stmt->bind_param("i", $id);
            $stmt->execute();
...
<body>
    ...
    <main class="main-content">
        <header class="page-header">
            ...
            <p class="page-description">จัดการข้อมูลผู้ใช้งานและกำหนดสิทธิ์</p>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <input type="text" id="username" name="username" required placeholder="e.g., user_gmesh">
                        </div>
                    </div>
