...
function getUserServiceFilePaths(data) {
...
function viewDetail(data) {
    ...
    let fileHtml = '';
    const filePaths = getUserServiceFilePaths(data);

    if (filePaths.length) {
        const links = filePaths.map((filePath, index) => `
            <a href="uploads/${filePath}" target="_blank" class="file-link">
                <i class="fas fa-file-download"></i> เปิดไฟล์แนบ ${index + 1}
            </a>
        `).join('');
        ...
        fileHtml = `
            <div class="attachment-box">
                ...
                <div style="display:flex; flex-direction:column; gap:10px;">${links}</div>
            </div>
        `;
    }
